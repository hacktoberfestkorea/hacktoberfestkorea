...
* 풀 리퀘스트는 GitHub에 호스팅 중인 저장소 및 프로젝트에서 만들 수 있습니다.
* 메인테이너들은 [`hacktoberfest` 토픽으로 프로젝트를 분류](https://do.co/hfest-topics)하여 참여를 선택할 수 있습니다.
* 여러분의 풀 리퀘스트는 `hacktoberfest` 토픽으로 분류된 저장소에 Merge되거나, 메인테이너의 승인을 받거나 `hacktoberfest-accepted` 라벨이 지정된 경우에 참여에 포함됩니다.
* 또한 `hacktoberfest` 토픽으로 분류 여부와 관계없이 GitHub 공개 저장소 중에서 `hacktoberfest-accepted` 라벨이 붙여진 풀 리퀘스트는 Hacktoberfest에 유효한 것으로 간주합니다.
* 10월 1일부터 10월 31일까지 참여자로 등록할 수 있습니다. 공식 Hacktoberfest 웹 사이트에 지금 등록해 풀 리퀘스트 카운트를 쌓아보세요.

# 참여 규칙
기념품인 셔츠를 받기 위해서는 10월 1일부터 31일(시간대 제약 없음)까지 풀 리퀘스트(PR)을 생성해야 합니다.

풀 리퀘스트는 GitHub에 공개된 어떠한 저장소도 괜찮습니다. [2020년 10월 3일 12:00(UTC)](hacktoberfest_spam_update.md)에 규칙이 변경되기 전의 풀 리퀘스트는 하이라이트 된 저장소뿐만 아니라 모든 GitHub 공개 저장소에 해당합니다.

규칙 변경에 따른 풀 리퀘스트는 반드시 GitHub 공개 저장소에서 이루어져야 하고 merge되어야 하며 `hacktoberfest-accepted` 라벨이 붙여지거나 승인을 받아야 합니다.

`hacktoberfest` 토픽에 해당하는 것과 관계없이 GitHub의 모든 공개 저장소에서 `hacktoberfest-accepted` 라벨이 지정된 풀 리퀘스트는 Hacktoberfest에 포함됩니다.

풀 리퀘스트에는 참여자 본인이 직접 커밋한 내역이 포함되어야 합니다. 메인테이너가 참여자의 풀 리퀘스트를 스팸으로 지정하면 Hacktoberfest는 참여자가 참여한 풀 리퀘스트로 인정하지 않습니다. 메인테이너가 참여자의 행동이 Hacktoberfest의 가치나 CoC(Code of Conduct, 행동 강령)에 위배됨을 보고하면 참여자의 참여 자격을 상실합니다. 올해는 선착순 7만 명의 참여자가 티셔츠를 얻거나 나무를 심을 수 있습니다.
